...
impl CurveEditor {
    ...
    fn install_draw(&self) {
        ...
        let floor = self.floor_pwm.clone();
        self.area.set_draw_func(move |_, cr, width, height| {
            ...
            // High-TDP floor.
            let floor_pwm = floor.get();
            if floor_pwm > 0 {
                cr.set_source_rgba(1.0, 0.5, 0.0, 0.55);
                cr.set_dash(&[4.0, 4.0], 0.0);
                cr.move_to(left, y(floor_pwm));
                cr.line_to(left + cw, y(floor_pwm));
                let _ = cr.stroke();
                cr.set_dash(&[], 0.0);
                cr.set_font_size(10.0);
                cr.move_to(left + 4.0, y(floor_pwm) - 4.0);
                let _ = cr.show_text("80% floor");
            }
    ...
    fn install_input(&self) {
        ...
        keys.connect_key_pressed(move |_, key, _, modifiers| {
            ...
            curve::enforce_curve(
                &mut curve,
                idx,
                editor.floor_pwm.get(),
                editor.clamp_grid.get(),
            );
            drop(curve);
            editor.emit_changed();
    ...
    fn move_point(&self, idx: usize, px: f64, py: f64, vertical_only: bool) {
        ...
        if vertical_only {
            ...
            curve::shift_curve_vertical(&mut curve, delta, self.floor_pwm.get());
        } else {
            curve[idx][0] = temp;
            curve[idx][1] = pwm;
            curve::enforce_curve(&mut curve, idx, self.floor_pwm.get(), self.clamp_grid.get());
        }
        drop(curve);
        self.emit_changed();
    }
