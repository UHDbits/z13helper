//! Fan curve math: validation, monotonic enforcement, clamp-to-grid, 80% floor.
//!
...
pub const HIGH_TDP_MIN_PWM: i32 = 204; // 80% of 255
...
pub enum CurveError {
    ...
    PwmDecreasing,
    #[error("PWM {pwm} at {temp}°C is below minimum {min} (80%)")]
    BelowFloor { pwm: i32, temp: i32, min: i32 },
}

/// Minimum PWM when PL1 exceeds the safe sustained max.
...
/// Validate against the high-TDP fan floor (daemon-matching wording).
pub fn validate_against_floor(curve: &Curve, pl1: u32) -> Result<(), CurveError> {
    validate(curve)?;
    let min = floor_pwm(pl1);
    if min > 0 {
        for pt in curve {
            if pt[1] < min {
                return Err(CurveError::BelowFloor {
                    pwm: pt[1],
                    temp: pt[0],
                    min,
                });
...
pub fn enforce_curve(curve: &mut Curve, idx: usize, min_pwm: i32, clamp_to_grid: bool) {
    if idx >= POINT_COUNT {
        return;
    }
    ...
    clamp_point(&mut curve[idx], min_pwm);
...
/// Shift every point's PWM by `delta`, clamped to `[min_pwm, PWM_MAX]`.
pub fn shift_curve_vertical(curve: &mut Curve, delta: i32, min_pwm: i32) {
    for pt in curve.iter_mut() {
        pt[1] = (pt[1] + delta).clamp(min_pwm, PWM_MAX);
    }
}
...
mod tests {
    ...
    #[test]
    fn floor_above_75() {
        assert_eq!(floor_pwm(75), 0);
        assert_eq!(floor_pwm(76), 204);
        assert_eq!(floor_pwm(93), 204);
    }
    ...
    fn default_curve_valid() {
        ...
        validate(&c).unwrap();
        validate_against_floor(&c, 60).unwrap();
    }

    #[test]
    fn default_curve_fails_high_tdp_floor() {
        let c = default_fan_curve();
        let err = validate_against_floor(&c, 80).unwrap_err();
        assert!(matches!(err, CurveError::BelowFloor { .. }));
    }
    ...
    #[test]
    fn enforce_respects_floor() {
        let mut c = default_fan_curve();
        enforce_curve(&mut c, 0, HIGH_TDP_MIN_PWM, false);
        for pt in &c {
            assert!(pt[1] >= HIGH_TDP_MIN_PWM);
        }
    }
    ...
    fn shift_vertical() {
        ...
        shift_curve_vertical(&mut c, 50, 0);
        assert_eq!(c[0][1], 150);
        shift_curve_vertical(&mut c, 200, 0);
